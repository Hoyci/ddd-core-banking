...
func (r *OutboxRepository) FindPending() ([]outbox.OutboxRecord, error) {
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("querying pending outbox records: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.ID, &record.EventName, &record.Payload, &record.OccurredAt); err != nil {
			return nil, fmt.Errorf("scanning outbox record: %w", err)
		}
...
func (r *OutboxRepository) MarkAsProcessed(id string) error {
	...
	`, time.Now(), id)
	if err != nil {
		return fmt.Errorf("marking outbox record %s as processed: %w", id, err)
	}
	return nil
}
